...
// Функтор для сортировки ходов: взятия дорогих фигур первыми.
// C++98: нет лямбд — функтор с operator() захватывает контекст через поле.
        ...
        for (int c = 0; c < 8; c++) {
            ...
            for (size_t i = 0; i < dests.size(); i++) {
                ...
                if (isPromo) {
                    ...
                    for (int p = 0; p < 4; p++) {
                        Move m;
                        m.fr = r; m.fc = c; m.tr = tr; m.tc = tc;
                        m.promotion = promos[p];
                        moves.push_back(m);
                    }
                } else {
                    Move m;
...
int AI::minimax(const Table& t, int depth, int alpha, int beta) const {
    ...
    for (size_t i = 0; i < moves.size(); i++) {
        ...
        Table copy = t; // глубокое копирование: 64 вызова clone()
        ...
        if (beta <= alpha) break; // α-β отсечение
    }
    return best;
}
...
Move AI::getBestMove(const Table& board, Color::E color) const {
    ...
    if (moves.empty()) {
        Move none;
        none.fr = -1; none.fc = -1; none.tr = -1; none.tc = -1; none.promotion = 'Q';
        return none;
    }
    bool maxing = (color == Color::White);
    int  best   = maxing ? INT_MIN : INT_MAX;
