...
// Парсит клетку из строки "e4" → Position(3, 4). Возвращает (-1,-1) при ошибке.
...
int main() {
    ...
    while (!board.isGameOver()) {
        ...
        std::cout << turnName;
        if (board.isInCheck(turn)) std::cout << " (ШАХ!)";
        std::cout << " > ";
        ...
        if (input == "quit" || input == "exit") { std::cout << "Выход.\n"; return 0; }
        if (input == "help") {
            std::cout << "Введите ход: e2e4. Для превращения: e7e8q (q/r/b/n).\n";
            continue;
        }

        if (input.size() < 4) { std::cout << "Неверный ввод. Пример: e2e4\n"; continue; }
